...
valueToByteStringBuilder :: Value -> Builder
valueToByteStringBuilder v = case v of
                               Null     -> null
                               Bool b   -> bool b
                               Number n -> number n
                               String s -> string s
...
array v
  ...
  | otherwise = B.char8 '[' <>
                valueToByteStringBuilder (V.unsafeHead v) <>
                V.foldr withComma (B.char8 ']') (V.unsafeTail v)
  where
    withComma a z = B.char8 ',' <> valueToByteStringBuilder a <> z
...
object m = case HMS.toList m of
  ...
  where
    ...
    one (k,v)     = string k <> B.char8 ':' <> valueToByteStringBuilder v
